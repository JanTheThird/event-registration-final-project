  ...
  if (saved) {
    ...
    return {
      ...
      users: parsed.users.map((user) => ({
        ...
        password:
          user.password ??
          fallbackPasswordsByEmail.get(user.email.toLowerCase()) ??
          'password123',
      })),
...
function updateEventQuota(eventId: number): void {
  const event = db.events.find((e) => e.id === eventId);
  if (!event) return;

  const registrationCount = db.registrations.filter((r) => r.eventId === eventId).length;
  event.registeredCount = registrationCount;
}
...
const addNotification = (
  ...
  const nextId =
    db.notifications.length > 0 ? Math.max(...db.notifications.map((n) => n.id)) + 1 : 1;
...
const getUserRegistrations = (userId: number): number[] =>
  db.registrations.filter((r) => r.userId === userId).map((r) => r.eventId);
...
const registerForEvent = (userId: number, eventId: number): boolean => {
  ...
  updateEventQuota(eventId);
  saveDB();
  return true;
};

const unregisterFromEvent = (userId: number, eventId: number): boolean => {
  const wasRegistered = db.registrations.some((r) => r.userId === userId && r.eventId === eventId);
...
/** Stable singleton — safe to list in React hook dependency arrays. */
const dbApi = {
  getUsers,
  getAllUsers,
  findUser,
